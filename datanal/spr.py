...
from ._utils import set_fig, scale_tick, read_curves, determine_unit, transform_unit
...
def curves_plot(
    ...
    if affinity_file is None:
        ...
        if K_d_mean is None or K_d_std is None:
            ax.set_title(
                f'Compound {os.path.basename(output_file)[:-5].replace("_", " ")}: Fitted unsuccessfully', y=1.05)
        else:
            ax.set_title(f'Compound {os.path.basename(output_file)[:-5].replace("_", " ")}: ' +
...
def main() -> None:
    ...
    parser.add_argument(
        'input_dir', help='Path to the folder containing the affinity data and SPR curves.')
    parser.add_argument('-a', '--affinity', action='store_true',
                        help='Whether to plot affinity.')
    parser.add_argument('-d', '--detect_affinity_file',
                        action='store_true', help='Whether to detect affinity file.')
