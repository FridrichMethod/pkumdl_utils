...
from FigureSettings import set_fig, scale_tick, determine_unit


def langmuir(x: np.ndarray, R_m: float, K_d: float, offset: float) -> np.ndarray:
    '''
    Define equation to fit affinity. Here we use the simplest form of the Langmuir 1:1 binding model.
...
def langmuir_fit(xdata: np.ndarray, ydata: np.ndarray) -> tuple[np.ndarray, np.ndarray] | tuple[None, None]:
    '''
    Fit affinity using the Langmuir 1:1 binding model with the initial guess and bounds.
    ...
    try:
        ...
        popt, pcov = scipy.optimize.curve_fit(
            langmuir, xdata, ydata, p0=initial_guess, bounds=(lower_bounds, upper_bounds))

        # Check the standard error of the fitted parameters
        perr = np.sqrt(np.diag(pcov))
        if perr[1] > popt[1] or np.sum(perr > popt) > 1:
            print('Warning: Too large SEM!')
            return None, None
    except RuntimeError:
...
def affinity_fit(affinity_file: str) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, float, float, str, float] | tuple[np.ndarray, np.ndarray, None, None, None, None, str, float]:
    '''
    Fit affinity using the Langmuir 1:1 binding model.
    ...
    if K_d_mean > 1e-4:
        print(
            f'Warning: {os.path.basename(affinity_file)} has too large dissociation constant.')
    if K_d_mean < np.min(xdata) or K_d_mean > (np.max(xdata) + np.min(xdata)) / 2:
        print(
            f'Warning: {os.path.basename(affinity_file)} has dissociation constant out of confident interval.')

    return xdata*factor, ydata, xfit*factor, yfit, K_d_mean*factor, K_d_std*factor, unit, factor


def affinity_plot(
    input_file: str, output_file: str, config_file: str,
    ...
    xdata, ydata, xfit, yfit, K_d_mean, K_d_std, unit, factor = affinity_fit(
        input_file)
...
def curves_plot(
    ...
    fig_size: tuple = (4.5, 3), dpi: float = 300, custom_palette: str | list[str] = 'bright', line_width: float = 2, bound: float = 0.5
) -> None:
    '''
    Plot SPR curves.
    ...
    try:
        ...
        ydata_max = np.max(scipy.signal.medfilt(
            df_y.values.T.flatten(), max_filter_order))
        for i, c in enumerate(conc):
            xfiltered: np.ndarray = np.array(df_x.iloc[:, i].values)
            ytofilter: np.ndarray = np.array(df_y.iloc[:, i].values)
            yfiltered = scipy.signal.medfilt(ytofilter, min_filter_order)
            ...
            for filter_order in range(min_filter_order+2, max_filter_order+2, 2):
                if np.max(np.abs(yfiltered[np.abs(xfiltered) < 1])) < peak_cutoff*ydata_max:
                    break
                yfiltered = scipy.signal.medfilt(ytofilter, filter_order)
            else:
                print(
                    f'Warning: Concentration {c} in {os.path.basename(output_file)} has reached the max filter order.')
    ...
    if affinity_file is None:
        ...
        K_d_mean, K_d_std, unit, factor = affinity_fit(affinity_file)[4:]
...
def affinity(input_dir: str, config_file: str, **kwargs) -> None:
    '''
...
def curves(input_dir: str, config_file: str, detect_affinity_file: bool = False, **kwargs) -> None:
    '''
        ...
        for file in os.listdir(input_dir):
            if file.endswith('c.txt'):
                if os.path.exists(os.path.join(input_dir, file[:-5] + 'a.txt')):
                    ...
                    print(
                        f'Warning: {os.path.basename(file)} does not have affinity file. Parsed affinity file will be used if exists.')
                try:
                    curves_plot(os.path.join(input_dir, file), os.path.join(
                        input_dir, file[:-4] + '.png'), config_file, **kwargs)
                except:
                    print(
                        f'Warning: Unknown errors occur. {os.path.basename(file)} is not plotted successfully and skipped.')
...
if __name__ == '__main__':
    ...
    affinity(input_dir, 'config.json')
    curves(input_dir, 'config.json')
